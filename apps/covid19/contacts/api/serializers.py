...
class UserContactTaggingSerializer(ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        today_contact = UserContacts.objects.filter(Q(user=user) | Q(user=contact_user),
                                                    Q(user_contacted=user) | Q(user_contacted=contact_user),
        ...
        if today_contact:
            ...
            if morning_tag and morning_tag.is_infected is False:
                # infected_contact = UserContacts.objects.filter(Q(user=contact_user) | Q(user_contacted=contact_user),
                #                                                date_contacted=validated_data.get(
                #                                                    "date_contacted"),
                #                                                is_infected=True).last()
                if user.risk_level is True or contact_user.risk_level is True:
                    if user.risk_level is False and contact_user.risk_level is True:
                        user.risk_level = True
                        user.contact_exposure += 1
                        user.save()
                        ...
                        if qs.exists():
                            ...
                            history = NotificationHistory(user=user, requested_user=user,
                                                          ...
                                                          title=title)
                            history.save()
                    if user.risk_level is True and contact_user.risk_level is False:
